...
using GlyphInstance = Render::GlyphInstance;
...
class Render::Vulkan {
   ...
   private:
    ...
    // tf is that assumption? - 2025/01/13 00:11
    ...
    void CreateVertexBuffer(VkBuffer &vertexBuffer, VkDeviceMemory &vertexBufferMemory, bool &haveVertexData, VkDeviceSize *vertexBufferSize, std::vector<Vertex> &vertices) {
    ...
    void CreateIndexBuffer(VkBuffer &indexBuffer, VkDeviceMemory &indexBufferMemory, std::vector<uint32_t> &indices) {
        if (indexBuffer != VK_NULL_HANDLE) {
            ...
            indexBuffer = VK_NULL_HANDLE;  // fixed sus
        }
    ...
    void CreateDummyBuffer(VkBuffer &dummyBuffer, VkDeviceMemory &dummyBufferMemory) {
...
Render::Render(bool debugging) { instance = new Render::Vulkan(debugging); }
